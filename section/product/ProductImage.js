        ...
        <Grid item xs={12} sm={12} md={6}>
            <Grid container px={1}>
                <Box display={'flex'} flexDirection={'column'} margin={'auto'}>
                    <Grid px={0} mt={mt}>
                        <Image src={product.images[bigImage-1].image} width={350} height={350} alt={'head phone'} className={"imageW-H"}/>
                    </Grid>
                    <Grid mt={margin}>
                            ...
                            {product.images.map(img => (
                                <Grid item key={img.id}>
                                    <PaperCustom sx={{width: '70px',padding:'0',border:img.id === bigImage ? `1px solid ${Theme.palette.primary.main}` : '',cursor:'pointer'}}>
                                        <Image src={img.image} alt={`${product.name}`} onClick={()=>setBigImage(img.id)} className={"imageW-H"}/>
                                    </PaperCustom>
                                </Grid>
                            ))}
                        </Grid>
